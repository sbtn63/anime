...
def get_favorite_list(favorite_list_id : int, user_id : int):
    with engine.connect() as conn:
        result = conn.execute(
            favorite_lists.select()
            .where(favorite_lists.c.id == favorite_list_id)
            .where(favorite_lists.c.user_id == user_id)
        ).first()
...
async def create_favorite_list(favorite_list_data : FavoriteListAddSchema, current_user : UserSchema = Depends(get_current_user)):
    with engine.connect() as conn:
        ...
        result = conn.execute(
            favorite_lists.insert()
            .values(favorite_list_data_dict)
        )
        conn.commit()
        
        last_inserted_id = result.inserted_primary_key[0]
        favorite_list = conn.execute(
            favorite_lists.select()
            .where(favorite_lists.c.id == last_inserted_id)
        ).fetchone()
...
async def update_favorite_list(id: int, favorite_list_data: FavoriteListUpdateSchema, current_user: UserSchema = Depends(get_current_user)):
    with engine.connect() as conn:
        ...
        conn.execute(
            favorite_lists.update()
            .where(favorite_lists.c.id == id)
            .values(update_data)
        )
        conn.commit()

        updated_favorite_list = conn.execute(
            favorite_lists.select()
            .where(favorite_lists.c.id == id)
        ).fetchone()
...
async def favorite_list_animes_list(id : int, current_user : UserSchema = Depends(get_current_user)):
    with engine.connect() as conn:
        ...
        result = conn.execute(
            ...
            .where(favorite_list_animes.c.favorite_list_id == id)
        ).fetchall()
...
async def add_anime_favorite_list(add_anime_favorite_data : FavoriteListAnimeAddSchema, current_user : UserSchema = Depends(get_current_user)):  
    try:
        ...
        with engine.begin() as conn:
            ...
            anime_exists = conn.execute(
                animes.select()
                .where(animes.c.id == data_anime['id'])
            )
            ...
            if anime_exists.first() is None:
                ...
                for item in data_genres:
                    conn.execute(
                        anime_genders.insert()
                        .values({"anime_id": anime_id, "gender_id": item['id']})
                    )
            
            conn.execute(
                favorite_list_animes.insert()
                .values(add_anime_favorite_data.dict())
            )
